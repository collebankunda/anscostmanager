...
# Define the expected columns for the actual expenses file.
EXPECTED_COLUMNS = ["Date", "Description", "Amount", "Cost Category"]
...
# SESSION STATE for expense entry – initialize with the expected columns.
if "expenses_data" not in st.session_state:
    st.session_state["expenses_data"] = pd.DataFrame(columns=EXPECTED_COLUMNS)

# If an actual file is uploaded, use that; otherwise, use session state.
if actual_df_uploaded is not None:
    actual_df = actual_df_uploaded.copy()
    actual_df.columns = actual_df.columns.str.strip()
    # Force actual_df to have exactly the expected columns.
    actual_df = actual_df.reindex(columns=EXPECTED_COLUMNS)
else:
    if not st.session_state["expenses_data"].empty:
        actual_df = st.session_state["expenses_data"].copy()
    else:
        actual_df = None

# STANDARDIZE COLUMN NAMES for budget file.
if budget_df is not None:
    budget_df.columns = budget_df.columns.str.strip()
    # Ensure budget file has "Cost Category" and rename "Amount" to "Amount_Budget"
    if "Category" in budget_df.columns:
        budget_df.rename(columns={"Category": "Cost Category"}, inplace=True)
    if "Cost Category" not in budget_df.columns:
        # Assume it already is "Cost Category" if not "Category"
        pass
    if "Amount" in budget_df.columns:
        budget_df.rename(columns={"Amount": "Amount_Budget"}, inplace=True)
    # Optionally, reindex to a known order (if needed)
    budget_df = budget_df.reindex(columns=["Cost Category", "Amount_Budget"])
...
with tab1:
    ...
    with st.form("expense_entry_form", clear_on_submit=True):
        ...
        # Allow any valid figure for amount (removing min_value and step restrictions)
        amount = st.number_input("Amount (UGX)", value=0.0, format="%.2f")
        category_options = [
        ...
        # Since the expected column is "Cost Category", we use that label.
        cost_category = st.selectbox("Cost Category", category_options)
        submitted = st.form_submit_button("Add Expense")
        if submitted:
            if description.strip() == "":
                st.warning("Please enter a valid description.")
            else:
                # Build the new expense row using the expected columns.
                ...
                # Get the current expenses DataFrame (it should already have the expected columns).
                current_expenses = st.session_state["expenses_data"].copy()
                # Append the new row.
                if current_expenses.empty:
                    updated_expenses = new_row_df
                else:
                    updated_expenses = pd.concat([current_expenses, new_row_df], ignore_index=True)
    ...
    # For export, we simply ensure the DataFrame has the expected columns.
    ...
    if "Cost Category" in budget_df.columns and "Cost Category" in actual_df.columns:
        ...
        with tab6:
            ...
            if "Date" in actual_df.columns:
                ...
                quarterly_trend = filtered_actual.groupby("Quarter")["Amount"].sum().reset_index()
            ...
            def generate_pdf_report():
                ...
                for idx, row in merged_df.iterrows():
                    pdf.cell(
                        200,
                        10,
                        f"{row['Cost Category']}: UGX {row['Amount']:,} (Variance: UGX {row['Variance']:,})",
                        ln=True
                    )
        ...
        with tab8:
            ...
            if total_input_weight == 0:
                ...
                merged_df["Normalized_Weighted_Actual"] = merged_df.apply(
                    lambda row: row["Amount"] * normalized_weights.get(row["Cost Category"], 0), axis=1
                )
